...
export function WritingWorkspace({ copy, feedbackCopy, prompt }: { copy: Copy; feedbackCopy: FeedbackCopy; prompt: string }) {
  ...
  async function evaluate(id: string) {
    ...
    if (!result.feedback) setMessage(feedbackCopy.feedbackUnavailable);
  }
  ...
  async function save(action: "SAVE_DRAFT" | "SUBMIT") {
    ...
    try {
      const response = await fetch("/api/writing/submissions", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ submissionId, promptId: "local-general-opinion", taskType: "GENERAL", promptText: prompt, text, action, examType: null }) });
      const body = await response.json() as { id?: string; error?: string };
      if (!response.ok || !body.id) throw new Error(body.error ?? "Save failed");
      setSubmissionId(body.id);
      if (action === "SUBMIT") await evaluate(body.id);
