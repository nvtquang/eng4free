...
export const vocabularyReviews = pgTable("vocabulary_reviews", { id: uuid("id").primaryKey(), ownerKey: varchar("owner_key", { length: 64 }).notNull(), userId: uuid("user_id"), guestId: uuid("guest_id"), vocabularyId: uuid("vocabulary_id").notNull().references(() => vocabulary.id, { onDelete: "cascade" }), dueAt: timestamp("due_at", { withTimezone: true }).notNull(), lastReview: timestamp("last_review", { withTimezone: true }), difficulty: real("difficulty").notNull(), stability: real("stability").notNull(), retrievability: real("retrievability").notNull(), elapsedDays: integer("elapsed_days").notNull(), scheduledDays: integer("scheduled_days").notNull(), learningSteps: integer("learning_steps").notNull(), repetitions: integer("repetitions").notNull(), lapses: integer("lapses").notNull(), fsrsState: integer("fsrs_state").notNull(), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow() }, (table) => [uniqueIndex("vocabulary_reviews_owner_vocab_idx").on(table.ownerKey, table.vocabularyId), index("vocabulary_reviews_owner_due_idx").on(table.ownerKey, table.dueAt)]);
